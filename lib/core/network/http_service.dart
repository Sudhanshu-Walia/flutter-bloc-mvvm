...
class HttpService {
  ...
  Future<ResponseWrapper<T?>> request<T>({
    ...
    try {
      ...
      var dataResponse = ResponseWrapper<T>(
        errorMessage: response.data["errorMessage"] ?? response.data["msg"],
      ...
      if (err is DioException) {
        printLog("response status :-> ${err.response?.statusCode}");
      ...
      if (res != null && res?.data.runtimeType != String) {
        ...
        return ResponseWrapper<T>(
          ...
          errorMessage:
              data["errorMessage"] ?? data["msg"] ?? "Something went wrong",
          msg: data["msg"],
          data: null,
        );
      }
  ...
  Future<Response> _checkRequest({
    ...
    if (requestType == RequestType.get) {
      ...
      final requestData =
          bodyType == BodyType.formData ? FormData.fromMap(body) : body;
      return _dio.post(
        fullUrl,
        data: requestData,
